...
const PerformanceLogManager = () => {
  ...
  const fetchPerformanceLogs = async () => {
    try {
      const response = await axios.get(
        `https://smart-bin-backend-production.up.railway.app/api/service/${userId}/performance-logs`,
        {
  ...
  const updatePerformanceLog = async () => {
    try {
      ...
      await axios.put(
        `https://smart-bin-backend-production.up.railway.app/api/service/${userId}/performance-logs/${editLog._id}`,
        editLogData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const deletePerformanceLog = async (logId) => {
    try {
      await axios.delete(
        `https://smart-bin-backend-production.up.railway.app/api/service/${userId}/performance-logs/${logId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  return (
    <div>
      ...
      <ul>
        {logs.map((log) => (
          <li key={log._id}>
            House Number: {log.houseNumber}, Waste Type: {log.wasteType}, Days:{" "}
            {log.days}, Date: {log.date}, Time: {log.time}
            <button onClick={() => handleEdit(log)}>Edit</button>
            <button onClick={() => deletePerformanceLog(log._id)}>
              Delete
            </button>
          </li>
        ))}
